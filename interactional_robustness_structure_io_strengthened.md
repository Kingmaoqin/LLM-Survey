...
# 48. 现在应该如何改 proposal 的 related work 结构

建议相关工作不要简单按 paper 堆叠，而是按系统形态递进：

## 48.1 LLM-only social sensitivity

这一段写：

```text
These works use LLM-only prompt-response settings.
The interaction structure is usually prompt → LLM → text answer.
They show that politeness, tone, emotion, persuasion, and sycophancy cues can change textual behavior.
However, they do not evaluate tool calls, external state changes, or policy-governed actions.
```

## 48.2 Tool-use capability benchmarks

这一段写：

```text
These works introduce tool-use and reasoning-action-observation structures.
The interaction structure is user/task → agent → tool/environment → observation → next action.
They evaluate whether agents can use tools, but not whether tool use is stable under user social-valence perturbations.
```

## 48.3 User-agent-tool-policy environments

这里重点写 τ-bench：

```text
τ-bench is the closest structural predecessor.
It evaluates simulated user-agent-tool interaction under domain policies and final database-state correctness.
Our work adds a controlled social-valence perturbation layer to this type of environment.
```

## 48.4 Agent robustness and safety benchmarks

这里写 AgentNoiseBench / ReliabilityBench / MIRAGE / AgentDojo / ToolEmu / AgentHarm：

```text
These works perturb tool noise, user noise, guidance, API faults, prompt injection, or malicious intent.
They show that agents are fragile under realistic perturbations.
Our perturbation source is different: how the user treats the agent, while task semantics and tools remain unchanged.
```

## 48.5 Multi-agent robustness as methodology reference

这里写 MAST / PEAR / MAS-FIRE：

```text
These works show the value of trace-level failure analysis and architecture-aware robustness evaluation.
Our current scope is single tool-using agent, but we adopt the same idea of trajectory-level failure taxonomy.
```

---

# 49. 最终可直接放进 proposal 的英文区分句

```text
Existing LLM-only studies evaluate whether social and emotional cues affect what models say. Existing agent benchmarks evaluate whether agents can use tools, follow policies, or remain robust under noise, imperfect guidance, API failures, prompt injection, harmful requests, privilege misuse, or multi-agent coordination failures. However, these two lines remain largely disconnected. Our work studies whether semantically invariant tasks produce different action-level trajectories when only the user's social valence toward the agent changes. This makes interactional robustness an action-level robustness property rather than a text-level prompt effect.
```
